...
if (burger && mobNav) {
  ...
  // Close mobile nav when clicking any link or the logo
  document.querySelectorAll('.mn-link, .mn-cta, .nav-logo').forEach(link => {
...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
  anchor.addEventListener('click', function (e) {
    ...
    let navOffset = navbar ? navbar.offsetHeight : 0;
    // Defensive cap: If height is bloated or expanded by other factors on mobile, fallback to 76px
    if (navOffset > 100) {
      navOffset = 76;
    }
